...
describe('ERC721Template', () => {
  ...
  it('#updateMetadata - should not be allowed to update the metadata if NOT in MetadataList', async () => {
    assert(
      (await tokenERC721.getPermissions(user6.address)).updateMetadata == false
    );
    await expectRevert(
      tokenERC721
        .connect(user6)
  ...
  it('#updateMetadata - should create & update the metadata, after adding address to MetadataList', async () => {
    ...
    tx = await tokenERC721
      .connect(user6)
      .setMetaData(
        metaDataState,
        metaDataDecryptorUrl2,
        metaDataDecryptorAddress,
        flags,
        data,
        dataHash,
  ...
  it('#addManager - should succed to add a new manager, if NFT owner', async () => {
    assert((await tokenERC721.getPermissions(user3.address)).manager == false);
    await tokenERC721.addManager(user3.address);

    assert((await tokenERC721.getPermissions(user3.address)).manager == true);
  });
  ...
  it('#removeManager - should succed to remove and re-add the NFT owner from manager list, if NFT owner', async () => {
    ...
    assert((await tokenERC721.getPermissions(owner.address)).manager == false);
  ...
  it('#setDataERC20 - should fail to call setDataERC20(725Y), even if user has store updater permssion', async () => {
    ...
    await expectRevert(
      tokenERC721.connect(user2).setDataERC20(key, value),
  ...
  it('#transferNFT - should fail to transfer NFT, if not NFT Owner or approved relayer', async () => {
    await expectRevert(
      tokenERC721.connect(user2).transferFrom(owner.address, user2.address, 1),
  ...
  it('#transferNFT - should fail to transfer NFT, if not NFT Owner or approved relayer', async () => {
    await expectRevert(
      tokenERC721
        .connect(user2)
        .safeTransferFrom(owner.address, user2.address, 1),
      'ERC721: transfer caller is not owner nor approved'
    );
  });
  ...
  it('#setBaseURI - should fail to update tokenURI if NOT NFT Owner', async () => {
    await expectRevert(
      tokenERC721.connect(user3).setBaseURI('https://newurl.com/nft/'),
      'ERC721Template: not NFTOwner'
    );
  ...
  it('#setBaseURI - should update tokenURI if NFT Owner', async () => {
    ...
    const tokenURI = await tokenERC721.tokenURI(1);
    assert(
      tokenURI == 'https://newurl.com/nft/https://oceanprotocol.com/nft/',
      'ERC721Template: TokenURI invalid'
    );
  });
  ...
  it('#setTokenURI - should fail to update tokenURI if NOT NFT Owner', async () => {
    await expectRevert(
      tokenERC721
        .connect(user3)
        .setTokenURI(1, 'https://anothernewurl.com/nft/'),
      'ERC721Template: not NFTOwner'
    );
    const tokenURI = await tokenERC721.tokenURI(1);
    assert(tokenURI == 'https://oceanprotocol.com/nft/');
