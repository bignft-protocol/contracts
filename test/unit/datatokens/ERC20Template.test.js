...
describe('ERC20Template', () => {
  ...
  it('#startOrder - user should succeed to call startOrder on a ERC20 without publishFee', async () => {
    ...
    assert(
      ...
      'Invalid OPF balance, we should get 0.03 DTs'
    );
    assert(
    ...
    assert(
      (await erc20Token.balanceOf(await erc20Token.getPaymentCollector())) ==
        web3.utils.toWei('0.97'),
      'Invalid publisher reward, we should have 0.97 DT'
    );
  });
  ...
  it('#orderExecuted - provider should succeed to call orderExecuted on a ERC20', async () => {
    ...
    const providerMessage = ethers.utils.solidityKeccak256(
      ['bytes32', 'bytes'],
      [orderTxId, ethers.utils.hexlify(ethers.utils.toUtf8Bytes(providerData))]
    );
    const providerSignature = await web3.eth.sign(
      providerMessage,
      providerAddress
    );
    ...
    const consumerMessage = ethers.utils.solidityKeccak256(
      ['bytes'],
      [ethers.utils.hexlify(ethers.utils.toUtf8Bytes(consumerData))]
    );
    const consumerSignature = await web3.eth.sign(
      consumerMessage,
      consumerAddress
    );
