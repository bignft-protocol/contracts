...
describe('FactoryRouter', () => {
  ...
  beforeEach('init contracts for each test', async () => {
    ...
    fixedRateExchange = await FixedRateExchange.deploy(router.address);
  ...
  it('#oceanTokens - should confirm Ocean token has been added to the mapping', async () => {
    assert(
      (await router.isApprovedToken(oceanAddress)) == true,
      'oceanAddress is not an ocean token'
    );
    const contractOceanTokens = await router.getApprovedTokens();
    assert(
      contractOceanTokens.includes(web3.utils.toChecksumAddress(oceanAddress)),
      'oceanAddress not found in router.getApprovedTokens()'
    );
  });

  it('#addOceanToken - should add and remove new token address to the mapping if Router Owner', async () => {
    assert(
      (await router.isApprovedToken(newToken.address)) === false,
      'newToken.address is an ocean token'
    );
    await router.addApprovedToken(newToken.address);
    assert(
      (await router.isApprovedToken(newToken.address)) == true,
      'newToken.address is not an ocean token'
    );
    let contractOceanTokens = await router.getApprovedTokens();
    assert(contractOceanTokens.length > 1);
    assert(
      contractOceanTokens.includes(
        web3.utils.toChecksumAddress(newToken.address)
      ),
      'newToken.address not found in router.getApprovedTokens()'
    );
    ...
    assert(
      ...
      'newToken.address found in router.getApprovedTokens()'
    );
  });

  it('#addOceanToken - should fail to add a new token address to the mapping if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).addApprovedToken(newToken.address),
  ...
  it('#removeOceanToken - should fail to remove a new token address to the mapping if NOT Router Owner', async () => {
    ...
    assert((await router.isApprovedToken(newToken.address)) == true);
    await expectRevert(
      router.connect(user2).removeApprovedToken(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isApprovedToken(newToken.address)) === true);
  });
  ...
  it('#ssContracts - should confirm ssContract has been added to the mapping', async () => {
    assert(
      (await router.isSSContract(sideStaking.address)) == true,
      'sideStaking.address is not a SS Contract'
    );
    const contractSSContracts = await router.getSSContracts();
    assert(
      contractSSContracts.includes(
        web3.utils.toChecksumAddress(sideStaking.address)
      ),
      'sideStaking.address not found in router.getSSContracts()'
    );
  });

  it('#ssContracts - should add and remove new contract if Router Owner', async () => {
    assert(
      (await router.isSSContract(newToken.address)) === false,
      'newToken.address is already a SS Contract'
    );
    await router.addSSContract(newToken.address);
    assert(
      (await router.isSSContract(newToken.address)) == true,
      'newToken.address is not a SS Contract'
    );
    ...
    assert(
      ...
      'newToken.address not found in router.getSSContracts()'
    );
    ...
    assert(
      ...
      'newToken.address found in router.getSSContracts()'
    );
  });

  it('#ssContracts - should fail to add a new contract address to the mapping if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).addSSContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert(
      (await router.isSSContract(newToken.address)) === false,
      'newToken.address is a SS Contract'
    );
  });
  ...
  it('#ssContracts - should fail to remove a new contract address to the mapping if NOT Router Owner', async () => {
    ...
    await expectRevert(
      router.connect(user2).removeSSContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isSSContract(newToken.address)) === true);
  });
  ...
  it('#addFactory - should fail to add a new factory address to the mapping even if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).addFactory(user2.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.factory()) == factoryERC721.address);
  });
  ...
  it('#FixedRateContracts - should confirm fixedrate has been added to the mapping', async () => {
    assert(
      (await router.isFixedRateContract(fixedRateExchange.address)) == true,
      'fixedRateExchange.address is not a FixedRate Contract'
    );
    const contractSSContracts = await router.getFixedRatesContracts();
    assert(
      contractSSContracts.includes(
        web3.utils.toChecksumAddress(fixedRateExchange.address)
      ),
      'fixedRateExchange.address not found in router.getFixedRateContracts()'
    );
  });

  it('#FixedRateContracts - should add and remove new contract if Router Owner', async () => {
    assert(
      (await router.isFixedRateContract(newToken.address)) === false,
      'newToken.address is already a FixedRate Contract'
    );
    await router.addFixedRateContract(newToken.address);
    assert(
      (await router.isFixedRateContract(newToken.address)) == true,
      'newToken.address is not a FixedRate Contract'
    );
    let contractFixedRateContracts = await router.getFixedRatesContracts();
    assert(contractFixedRateContracts.length > 1);
    assert(
      contractFixedRateContracts.includes(
        web3.utils.toChecksumAddress(newToken.address)
      ),
      'newToken.address not found in router.getFixedRateContracts()'
    );
    ...
    assert(
      ...
      'newToken.address found in router.getFixedRateContracts()'
    );
  });

  it('#FixedRateContracts - should fail to add a new contract to the mapping if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).addFixedRateContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert(
      (await router.isFixedRateContract(newToken.address)) === false,
      'newToken.address is an FixedRate'
    );
  });
  ...
  it('#FixedRateContracts - should fail to remove a new contract address to the mapping if NOT Router Owner', async () => {
    ...
    await expectRevert(
      router.connect(user2).removeFixedRateContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isFixedRateContract(newToken.address)) === true);
  });
  ...
  it('#DispenserContracts - should confirm Dispenser has been added to the mapping', async () => {
    assert(
      (await router.isDispenserContract(dispenser.address)) == true,
      'dispenser.address is not a Dispenser Contract'
    );
    const contractSSContracts = await router.getDispensersContracts();
    assert(
      contractSSContracts.includes(
        web3.utils.toChecksumAddress(dispenser.address)
      ),
      'dispenser.address not found in router.getDispenserContracts()'
    );
  });

  it('#DispenserContracts - should add and remove new contract if Router Owner', async () => {
    assert(
      (await router.isDispenserContract(newToken.address)) === false,
      'newToken.address is already a Dispenser Contract'
    );
    await router.addDispenserContract(newToken.address);
    assert(
      (await router.isDispenserContract(newToken.address)) == true,
      'newToken.address is not a Dispenser Contract'
    );
    let contractDispenserContracts = await router.getDispensersContracts();
    assert(contractDispenserContracts.length > 1);
    assert(
      contractDispenserContracts.includes(
        web3.utils.toChecksumAddress(newToken.address)
      ),
      'newToken.address not found in router.getDispenserContracts()'
    );
    ...
    assert(
      ...
      'newToken.address found in router.getDispenserContracts()'
    );
  });

  it('#DispenserContracts - should fail to add a new contract address to the mapping if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).addDispenserContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert(
      (await router.isDispenserContract(newToken.address)) === false,
      'newToken.address is an Dispenser'
    );
  });
  ...
  it('#DispenserContracts - should fail to remove a new contract address to the mapping if NOT Router Owner', async () => {
    ...
    await expectRevert(
      router.connect(user2).removeDispenserContract(newToken.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isDispenserContract(newToken.address)) === true);
  });
  ...
  it('#addPoolTemplate - should fail to add a new pool template contract if NOT Router Owner', async () => {
    ...
    await expectRevert(
      router.connect(user2).addPoolTemplate(user2.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isPoolTemplate(user2.address)) == false);
  });
  ...
  it('#removePoolTemplate - should fail to remove pool template contract if NOT Router Owner', async () => {
    ...
    await expectRevert(
      router.connect(user2).removePoolTemplate(poolTemplate.address),
      'OceanRouter: NOT OWNER'
    );
    assert((await router.isPoolTemplate(poolTemplate.address)) == true);
  });
  ...
  it('#updateOPCCollector - should fail to update OPC Collector if NOT Router Owner', async () => {
    await expectRevert(
      router.connect(user2).updateOPCCollector(user2.address),
      'OceanRouter: NOT OWNER'
    );
  });
